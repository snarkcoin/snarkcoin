...
# snarkcoin_hash
RUN git clone https://github.com/snarkcoinpay/snarkcoin_hash
RUN cd snarkcoin_hash && python3 setup.py install
...
RUN mkdir /snarkcoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /snarkcoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /snarkcoin-src
