...
*Setup instructions for a Gitian build of Snarkcoin Core using a Debian VM or physical system.*
...
- [Building Snarkcoin Core](#building-snarkcoin-core)
...
Clone the git repositories for Snarkcoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/snarkcoinpay/snarkcoin
...
in the Snarkcoin Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Snarkcoin Core
----------------

To build Snarkcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the Snarkcoin Core repository.
...
```bash
    ...
    From https://github.com/snarkcoinpay/snarkcoin
...
and the snarkcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../snarkcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/snarkcoin
SIGPATH=/some/root/path/snarkcoin-detached-sigs

./bin/gbuild --url snarkcoin=${BTCPATH},signature=${SIGPATH} ../snarkcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/snarkcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/snarkcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/snarkcoin-osx-build.assert
```
...
[snarkcoin/gitian.sigs](https://github.com/snarkcoinpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
