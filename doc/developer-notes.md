...
- [Development guidelines](#development-guidelines)
    - [General Snarkcoin Core](#general-snarkcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Snarkcoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/snarkcoinpay/snarkcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_snarkcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_snarkcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/snarkcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_snarkcoin.coverage/index.html`.
```

**Sanitizers**

Snarkcoin Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 7556.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 7555 for connections and services them.
...
be sent upstream but bugfixes may also be prudent to PR against Snarkcoin Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `snarkcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `snarkcoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Snarkcoin Core, the wallet was always
