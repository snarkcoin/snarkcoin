Cross-compiliation of Snarkcoin Core
===============================

Snarkcoin Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building Snarkcoin.
...
When building Snarkcoin Core, use
...
Note that for WSL the Snarkcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/snarkcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
It's not possible to build the Snarkcoin Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Snarkcoin Core source code).
