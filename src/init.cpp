...
public:
    ...
    // Snarkcoin Specific WalletInitInterface InitCoinJoinSettings
 ...
 * between snarkcoind, and snarkcoin-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-sporkaddr=<snarkcoinaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", false, OptionsCategory::DEBUG_TEST);
    ...
    gArgs.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by Snarkcoin Platform, to the specified username.", false, OptionsCategory::MASTERNODE);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/snarkcoin/snarkcoin>";
    const std::string URL_WEBSITE = "<https://snarkco.in>";
...
/** Sanity checks
 *  Ensure that Snarkcoin Core is running in a usable environment with all
...
{
    // Make sure only a single Snarkcoin Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if Snarkcoin Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Snarkcoin Core is started while in a temporary directory.\n",
        ...
        do {
            ...
            try {
                ...
                if (!fDisableGovernance && !fTxIndex
                   && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/snarkcoin/snarkcoin/pull/1817 and https://github.com/snarkcoin/snarkcoin/pull/1743
    ...
    // ********************************************************* Step 10c: schedule Snarkcoin-specific tasks
