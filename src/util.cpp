...
const char * const BITCOIN_CONF_FILENAME = "snarkcoin.conf";
const char * const BITCOIN_PID_FILENAME = "snarkcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "snarkcoind -foo=bar
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Snarkcoin";
#else
    // Unix
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty snarkcoin.conf if it does not excist
